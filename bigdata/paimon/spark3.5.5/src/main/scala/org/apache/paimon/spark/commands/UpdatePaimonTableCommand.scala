...
case class UpdatePaimonTableCommand(
  ...
  private def writeUpdatedAndUnchangedData(
    ...
    val excluded = Set(ROW_ID_COLUMN, SEQUENCE_NUMBER_COLUMN)

    var updateColumns = updateExpressions.zip(relation.output)
      //.filterNot { case (_, origin) => excluded(origin.name) }
      .map { case (update, origin) =>
