...
ANALYZE TABLE 收集表大小、行数、列的 ndv/min/max/null/长度等统计。CBO 用这些统计做选择性估算，触发空结果优化、动态分区裁剪、最优 Join 顺序、Bloom Filter 注入、倾斜处理等。
...
见：SPARK-54725: Add inferring transitive join conditions
