...
* 目前版本 优化规则 实现比较简单：isSelectiveFilterOverScan
  - isSelectiveFilterOverScan 只匹配 Filter/Project + LeafNode 的简单模式，错过 Aggregate、SubqueryAlias、多层 Project、CTE 等常见场景下的选择性过滤。改进方向包括递归穿透包装节点、结合 CBO 统计量化选择性、以及支持 CTE 追溯。
