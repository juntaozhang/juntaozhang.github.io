...
ShuffleQueryStageExec，核心作用：

- 包装已完成的 Shuffle，
- 暴露运行时 MapOutputStatistics，让 AQE 能判断"实际数据大小"并动态切换策略。
...
| before                                             | after                                             |
| -------------------------------------------------- | ------------------------------------------------- |
| ![bytedance1](asset/OptimizeSkewedJoin_before.png) | ![bytedance2](asset/OptimizeSkewedJoin_after.png) |
...
当倾斜分区的绝大部分数据集中在上游单个 Mapper 的单个 Block 时，AQE 的 OptimizeSkewedJoin 规则会失效

> 因为 Spark 的 Shuffle 读取机制是 “要么读取整个 Mapper Block，要么不读”，无法只读取 Block 的一部分数据，导致倾斜分区无法被拆分。
>
> TODO 这里优化是否会很难？
...
比如如果 right_table 有 ORDER BY / CLUSTER BY 会让优化失效；或者原表本来就是根据join key排序

这会导致表的物理存储顺序完全按 user_id 排列，比如 user_id 全是 1,1,1,...。

Spark 读取文件时，每个 Mapper 会拿到一个“连续的范围”（一个 Split）。

由于数据是排序的，某个 Mapper 很可能会拿到一大段完全相同的 user_id， 这个Mapper就无法被重新组合。

#### Join 一侧后续是 aggr window

如果 Join 的发生倾斜的一侧存在 Agg 或者 Window 这类有指定 requiredChildDistribution 的算子，那么 SkewedJoin 优化无法处理，因为将分区切分会破坏 RDD 的 outputPartitioning，导致不再满足 requiredChildDistribution。

join 后有 Window, 本来直接分区内部排序，现在 join 后的结果已经不能满足 `ClusteredDistribution(user_id)`

```sql
Window [PARTITION BY user_id ORDER BY order_time]   ← 仍然需要这个分布
  └── Sort [order_time]
        └── ShuffleExchange [hash(user_id)]         ← 额外引入的 Shuffle！
              └── SortMergeJoin (倾斜拆分后的输出)     ← 分布已被破坏
                    ├── ...
                    └── ...
```
...
举例：有 100 个 1MB 的小文件，Spark 可能把它们合并成几个 FilePartition，每个分区加起来接近 128MB，然后分别由一个 Map 任务处理。
...
#### Outer/Semi Join 场景不支持

对于 Outer/Semi Join，AQE SkewedJoin 是无法处理非 Outer/Semi 侧的数据倾斜。

比如，对于 LeftOuter Join，SkewedJoin 无法处理右侧的数据倾斜。

LeftOuter为什么不能split right（右表倾斜）呢？ 假设可以：
...
#### [SPARK-56767](https://issues.apache.org/jira/browse/SPARK-56767) Optimize hot-key skew joins by splitting into a broadcast branch for hot keys and a sort-merge branch for the rest

v4.1.1 正在进行...

以前的土办法: 业务自己**加盐（salting）改 SQL** 拆 Key，写法麻烦、容易写错、侵入业务代码。

```sql
WITH
-- 1. 大表：只给倾斜key加盐，非倾斜key保持原样
behavior_salt AS (
    SELECT
        user_id,
        -- 核心加盐逻辑：只有倾斜key加随机后缀
        CASE
            WHEN user_id = '10086' THEN CONCAT(user_id, '_', FLOOR(RAND() * 10))  -- 盐 0~9
            ELSE user_id
        END AS join_key,
        col1, col2, col3  -- 你的其他字段
    FROM user_behavior
),

-- 2. 小表：只给倾斜key扩盐（生成10条匹配数据）
info_salt AS (
    SELECT
        user_id,
        -- 扩盐：只给倾斜key生成 10 个join_key
        CASE
            WHEN user_id = '10086' THEN CONCAT(user_id, '_', idx)
            ELSE user_id
        END AS join_key,
        u_col1, u_col2  -- 其他字段
    FROM user_info
...
-- 3. 最终 JOIN
SELECT b.*, u.*
FROM behavior_salt b
JOIN info_salt u
ON b.join_key = u.join_key;
```

**Hot-Key Split**：引入新的`SKEW_JOIN` Hint，在物理计划阶段就将Join一分为二：

1. 先抽样统计，找出高频**热点 Key**
2. **热点分支 (Hot Branch)**：将识别出的高频Key数据，通过**Broadcast Hash Join**在Map端快速完成处理。
3. **常规分支 (Normal Branch)**：所有非高频Key的数据，继续通过**Sort Merge Join**处理。
4. **合并结果**：最终将两个分支的结果进行Union。

```sql
-- =============================================
-- 第一段：倾斜 key → 走广播 join（超级快）
-- =============================================
SELECT /*+ BROADCAST(u) */ b.*, u.*
FROM user_behavior b
JOIN user_info u
  ON b.user_id = u.user_id
WHERE b.user_id IN ('10086')  -- 倾斜key

UNION ALL

-- =============================================
-- 第二段：非倾斜 key → 正常 join（不倾斜）
-- =============================================
SELECT b.*, u.*
FROM user_behavior b
JOIN user_info u
  ON b.user_id = u.user_id
WHERE b.user_id NOT IN ('10086');
```
